...
export default function AcceptedRequestsPage() {
  ...
  const getStatusLabel = (status: string) => {
    switch (status) {
      ...
      case 'driver_enroute': return 'Driver En-route';
      case 'with_customer': return 'With Customer';
      case 'towing_vehicle': return 'Towing Vehicle';
      case 'at_destination': return 'At Destination';
      case 'waiting_confirmation': return 'Waiting Confirmation';
  ...
  const getStatusClass = (status: string) => {
    switch (status) {
      ...
      case 'driver_enroute': return 'status-enroute';
      case 'with_customer': return 'status-with-customer';
      case 'towing_vehicle': return 'status-towing';
      case 'at_destination': return 'status-destination';
      ...
      <div className="card">
        ...
        <div className="responsive-list">
          {loading ? <p style={{ padding: '20px', color: '#666' }}>Loading...</p> : (
            requests.length === 0 ? (
              <p style={{ textAlign: 'center', padding: '20px', color: '#666' }}>No accepted requests found</p>
            ) : requests.map((req) => {
                  ...
                  <div className="list-item-summary" onClick={() => toggleReq(req.id)}>
                    <div className="list-item-info">
                      ...
                      <div className="info-col">
                        ...
                        <span className={`status-badge ${getStatusClass(req.status)}`} style={{ display: 'inline-block', width: 'fit-content' }}>
                    ...
                    <div className="details-grid">
                      ...
                      <div className="details-block">
                        ...
                        <span className="details-value">
                          {req.driverId ? (
                            <span className={`status-badge ${req.driverId.isOnline ? 'status-online' : 'status-offline'}`}>
                              {req.driverId.isOnline ? 'Online' : 'Offline'} (ID: {req.driverId.id})
                            </span>
                          ) : '-'}
                        </span>
                      </div>
