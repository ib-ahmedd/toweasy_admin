...
export default function DisputesPage() {
  ...
  const [resolvingRequest, setResolvingRequest] = useState<TowRequest | null>(null);
  ...
  const handleResolve = async (status: string, driverId?: string) => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:5000/api/requests/${resolvingRequest.id}`, {
        method: 'PATCH',
      ...
      <div className="card">
        ...
        <div className="responsive-list">
          {loading ? <p style={{ padding: '20px', color: '#666' }}>Loading...</p> : (
            disputes.length === 0 ? (
              <p style={{ textAlign: 'center', padding: '20px', color: '#666' }}>No active disputes found</p>
            ) : disputes.map((req) => {
              const isExpanded = !!expandedDisputes[req.id];
              return (
                <div key={req.id} className={`list-item ${isExpanded ? 'expanded' : ''}`}>
                  <div className="list-item-summary" onClick={() => toggleDispute(req.id)}>
                    <div className="list-item-info">
                      <div className="info-col">
                        <span className="info-label">Request ID</span>
                        <span className="info-value" style={{ fontFamily: 'monospace' }}>{req.id.substring(0, 8)}...</span>
                      </div>
                      <div className="info-col">
                      ...
                      <div className="info-col">
                        ...
                        <span className="info-value">
                          {req.driverId ? req.driverId.name : <span style={{ color: '#999', fontStyle: 'italic' }}>Unknown</span>}
                        </span>
                      </div>
                      <div className="info-col">
                        <span className="info-label">Status</span>
                        <span className="status-badge status-disputed" style={{ display: 'inline-block', width: 'fit-content' }}>
                          DISPUTED
                        </span>
                      </div>
                    </div>
                  ...
                  <div className="list-item-details">
                    <div className="details-grid">
                      <div className="details-block">
                        ...
                        <span className="details-value" style={{ fontFamily: 'monospace' }}>{req.id}</span>
                      </div>
                      <div className="details-block">
                      ...
                      <div className="details-block">
                        <span className="details-label">Driver Assigned Details</span>
                        <span className="details-value">
                          {req.driverId ? (
                            <span>{req.driverId.name} (ID: {req.driverId.id})</span>
                          ) : (
                            <span style={{ color: '#999', fontStyle: 'italic' }}>Unknown Driver</span>
                          )}
                        </span>
                      </div>
                      <div className="details-block">
                      ...
                      <div className="details-block">
                        ...
                        <span className="details-value">🕒 {new Date(req.createdAt).toLocaleString()}</span>
                      </div>
                    </div>
        ...
        <div className="modal-overlay">
          <div className="modal-content">
            ...
            <p style={{ margin: '10px 0', color: '#666' }}>
              Resolving request for <strong>{resolvingRequest.userName}</strong> ({resolvingRequest.vehicleType})
            </p>
            ...
            <div className="resolution-options">
              ...
              <div className="option-group" style={{ marginTop: '20px' }}>
                ...
                <div style={{ display: 'flex', gap: '10px', marginTop: '10px' }}>
                  <button 
                    className="btn" 
                    style={{ backgroundColor: '#28a745', color: 'white', flex: 1 }}
