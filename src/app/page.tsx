...
export default function AdminDashboard() {
  ...
  const fetchData = async () => {
    try {
      const [reqRes, appRes] = await Promise.all([
        fetch("https://toweasy-server.onrender.com/api/requests"),
        fetch("https://toweasy-server.onrender.com/api/applications"),
      ]);
  ...
  const stats = {
    ...
    active: requests.filter(
      (r) => r.status === "accepted" || r.status === "waiting_confirmation"
    ).length,
  ...
  return (
    <>
      ...
      <div className="stats-grid">
        ...
        <div
          className="stat-card"
          style={{ borderTopColor: "var(--pending-orange)" }}
        >
          <h4>Pending</h4>
          <p>{stats.pending}</p>
        </div>
        ...
        <div
          className="stat-card"
          style={{ borderTopColor: "var(--danger-red)" }}
        >
          <h4>Disputed</h4>
          <p>{stats.disputed}</p>
        </div>
        <div
          className="stat-card"
          style={{ borderTopColor: "var(--success-green)" }}
        >
          <h4>Completed</h4>
          <p>{stats.completed}</p>
        </div>
